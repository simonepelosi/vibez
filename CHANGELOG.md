...
### Added
- **Automatic Widevine CDM downloader mirroring Firefox** — Vibez now automatically fetches and extracts the official Google Widevine CDM component package directly from Google's CDN if no compatible browser is found on the system. Scans Firefox user profiles on both macOS and Linux for existing CDM installations. Eliminates any requirement to have Google Chrome or any browser installed on the system.
- **Browserless Apple Music playback on macOS via native Widevine CDM and AVFoundation** — extends the browserless audio engine to macOS (Darwin). Uses `libwidevinecdm.dylib` from installed macOS browsers (Google Chrome, Brave, Microsoft Edge, Arc, Firefox) or user cache, combined with native AVFoundation audio playback without requiring GStreamer or any external audio packages. Delivers identical low memory footprint (~15 MB) and sub-second startup on macOS.
...
- Thanks to @ianswope for enabling CGo on macOS release builds, developer token expiration recovery, and token TTL extensions.
...
### Fixed
- **MPRIS metadata leaked from one track to the next** — godbus merges each property update into the map it already holds rather than replacing it (`prop.Export` allocates the storage once at registration; `dbus.Store` → `storeMapIntoMap` does in-place `SetMapIndex` unless the destination is nil), so any key left out of an update silently kept its previous value. A track with no artwork advertised the *previous* track's `mpris:artUrl`, and because a nil track wrote only `mpris:trackid`, stopping playback left the previous title, artist, album, and length exported — visible in playerctl, waybar, and the GNOME/KDE media widgets. `flush` now writes the full key set on every update, with zero values where a field is absent. Removing keys outright rather than blanking them needs the property-export change tracked in #110.
- **`install.sh` refused to run on macOS** — the installer died with `vibez currently supports Linux only (got: darwin)`, and Apple Silicon hit a second, Linux-specific arm64 rejection right behind it, even though every release publishes `vibez_darwin_arm64.tar.gz` and `vibez_darwin_amd64.tar.gz`. macOS users had no binary install path at all: the one-liner refused them and building from source needs Apple Developer credentials. The platform check now accepts darwin on both architectures, the XDG desktop-entry and icon step is skipped outside Linux, and a macOS install ends by looking for Google Chrome in the same order `internal/player/cdp/browser_darwin.go` searches — `VIBEZ_CHROME_PATH`, `CHROME_PATH`, `/Applications`, `~/Applications` — warning with the `brew install --cask google-chrome` hint when it is missing. Refs #11.
- **Checksum verification aborted every macOS install** — macOS ships its own `/sbin/sha256sum` (`sha256sum (Darwin) 1.0`), which wins the `command -v sha256sum` branch but understands only `[-bctwz]`, so the GNU-style `--check --status` pipeline failed and reported `Checksum mismatch — the download may be corrupted` on a perfectly good archive. The installer now reads the expected digest out of `checksums.txt` by exact filename and compares it against a digest taken with the bare `sha256sum`/`shasum -a 256` invocation, which prints the same format on GNU coreutils, BSD and `shasum`. A missing checksum line is also a hard failure now instead of printing `✓ Checksum OK` regardless.
- **Quitting left the shell with no echo or ctrl+c** — `playwright.Run` piped the node driver's stdin and stdout but passed its stderr through to the terminal, so the driver held a live TTY for the whole session. Node snapshots that terminal's termios when it sets up the stream — after BubbleTea has switched to raw mode — and restores the snapshot when it exits, which happens during shutdown *after* BubbleTea has restored cooked mode, silently undoing it. The driver's stderr now goes to `io.Discard`, along with an explicit Playwright logger so that setting it does not also repoint Go's standard logger. Refs #88.
...
- Thanks to @ianswope for contributing the album-art view (`:art`), Linux/arm64 CDP playback, macOS install script support, MPRIS fixes, and build targets.
- Thanks to @elijah-rou for hardening MPRIS track identity updates.
